...
This template deploys RiotPlan MCP HTTP (`riotplan-mcp-http`) to Google Cloud Run using Cloud Build.
...
This matches your `ops/protokoll` workflow:
...
```bash
RIOTPLAN_VERSION=1.0.22-dev.0 ./deploy/cloud-run/deploy-prod.sh
```

## Local testing (kept intact)
